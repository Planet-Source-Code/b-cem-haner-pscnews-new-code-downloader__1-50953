Attribute VB_Name = "Tanimlar_ProApi"
...
Private Function AddControl(inControl As Control, inName As String) As Long
    ...
    Else
        ControlRecord(MaxControl).Top = inCmontrol.Top
...
Sub ProgressSetColor(lProgressBarHwnd As Long, lColour As Long, bForeground As Boolean)
...
Public Function ConnectVarmi() As Boolean
    
    
    If IsNetConnectViaModem = True Or IsNetConnectViaProxy = True Or IsNetConnectViaLAN = True Then
        ConnectVarmi = True
    Else
        ConnectVarmi = False
    End If
...
End With
End Sub
Function InternetDedect() As Boolean
   If InternetCheckConnection("http://www.pentasoft.com.tr/", FLAG_ICC_FORCE_CONNECTION, 0&) = 0 Then
        InternetDedect = False
    Else
        InternetDedect = True
    End If
End Function
